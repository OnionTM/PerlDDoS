...
if ($help || !$ip) {
 ...
 Klez.pl Criado para NeverSec By:Klez

 Klez.pl oficial da NeverSec

 Para utilizar use:
 perl Klez.pl IP PORTA 65500
...
print "Atacando ip: $ip " . ($port ? $port : "By:") . " -->Klez<-- " . 
  ($size ? "$size-byte" : "UDP Oficial da: NeverSec") . " " . ($time ? " for $time seconds" : "") . "\n";
